...
public class ChenileMapper {
    ...
    private static void mapProperty(BeanWrapper wrapper, String propertyName, Class<?> propertyType,
        ...
        if (!isSimpleType(propertyType)) {
            for (Field nestedField : propertyType.getDeclaredFields()) {
                ...
                // Try flattened patterns: propertyName + CamelCaseNestedField (e.g.,
                // priceAmount)
                ...
                // Try flattened snake pattern: property_name + _ + nested_field_name (e.g.,
                // price_amount)
                ...
                // Only try this if the target nested field doesn't clash with an existing
                // top-level property
                String lowerNested = nestedFieldName.toLowerCase();
                if (row.containsKey(lowerNested) && !hasTopLevelProperty(wrapper, nestedFieldName)) {
    ...
    private static boolean hasTopLevelProperty(BeanWrapper wrapper, String propertyName) {
        try {
            return wrapper.getPropertyType(propertyName) != null;
